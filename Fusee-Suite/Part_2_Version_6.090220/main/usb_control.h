...
void send_fusee(const byte *payload)
{
  ...
  usbBufferedWrite(FS1,81572);
  
  if (EEPROM_BOOTLOGO == 1) usbBufferedWrite(ANIM00,6);
  else if (EEPROM_BOOTLOGO == 2) usbBufferedWrite(ANIM01,6);
  else if (EEPROM_BOOTLOGO == 3) usbBufferedWrite(ANIM02,6);
  else if (EEPROM_BOOTLOGO == 4) usbBufferedWrite(ANIM03,6);
  else if (EEPROM_BOOTLOGO == 5) usbBufferedWrite(ANIM04,6);
  
  usbBufferedWrite(EMPTY1,32);
  
  if(EEPROM_SCREENSAVER_ACTIVE == 1) usbBufferedWrite(SCREENSAVER1,12);
  else usbBufferedWrite(SCREENSAVER0,12);

  usbBufferedWrite(EMPTY2,64);
  //usbBufferedWrite(EMPTY3,38);
  
  usbBufferedWrite(payload, 12);

  usbBufferedWrite(EMPTY4,102);
  
  if (EEPROM_PAYLOAD_NUMBER == 1) {usbBufferedWrite(DIS1,12);}
  if (EEPROM_PAYLOAD_NUMBER == 2) {usbBufferedWrite(DIS2,12);}
  if (EEPROM_PAYLOAD_NUMBER == 3) {usbBufferedWrite(DIS3,12);}
  if (EEPROM_PAYLOAD_NUMBER == 4) {usbBufferedWrite(DIS4,12);}
  if (EEPROM_PAYLOAD_NUMBER == 5) {usbBufferedWrite(DIS5,12);}
  if (EEPROM_PAYLOAD_NUMBER == 6) {usbBufferedWrite(DIS6,12);}
  if (EEPROM_PAYLOAD_NUMBER == 7) {usbBufferedWrite(DIS7,12);}
  if (EEPROM_PAYLOAD_NUMBER == 8) {usbBufferedWrite(DIS8,12);}

  usbBufferedWrite(EMPTY5,64);
  
  if(EEPROM_JOYCON_CONTROL_STRAP == 1) usbBufferedWrite(JOYCONINFO,11);
  else usbBufferedWrite(NOJOYCONINFO,11);

  usbBufferedWrite(EMPTY6,27);
  ...
  if (EEPROM_DUAL_BOOT_TOGGLE == 1) usbBufferedWrite(FSDUALBOOT,31);
    else usbBufferedWrite(FSAUTORCM,31);

  usbBufferedWrite(EMPTY7,7);

  if (EEPROM_BOOTLOGO == 1) usbBufferedWrite(BOOTANIM0,26);
  else if (EEPROM_BOOTLOGO == 2) usbBufferedWrite(BOOTANIM2,26);
  else if (EEPROM_BOOTLOGO == 3) usbBufferedWrite(BOOTANIM3,26);
  else if (EEPROM_BOOTLOGO == 4) usbBufferedWrite(BOOTANIM4,26);
  else if (EEPROM_BOOTLOGO == 5) usbBufferedWrite(BOOTANIM5,26);

  usbBufferedWrite(EMPTY8,12);

  if(EEPROM_SCREENSAVER_ACTIVE == 1) usbBufferedWrite(SSAVERON,22);
  else usbBufferedWrite(SSAVEROFF,22);

  usbBufferedWrite(EMPTY9,16);
  if (EEPROM_BITMAP_BOOTLOGO_ON == 1) usbBufferedWrite(LOGOENABLE,12);
  else usbBufferedWrite(LOGODISABLE,12);
  
 usbBufferedWrite(FS2,364);
 usbBufferedWrite(FS3,15148);
  usbFlushBuffer();
}
