...
uint8_t EEPROM_SCREENSAVER_ACTIVE;
uint8_t EEPROM_BOOTLOGO;
uint8_t EEPROM_BITMAP_BOOTLOGO_ON;
...
void readSettings()
{
  for (int y = PAGE_15; y >= (PAGE_00); y -= PAGE_SIZE)
  {
    ...
    EEPROM_SCREENSAVER_ACTIVE = config-> j1;
    EEPROM_BOOTLOGO = config-> k1;
    EEPROM_BITMAP_BOOTLOGO_ON = config-> l1;
...
void writeSettings()
{
  ...
  config.j1 = EEPROM_SCREENSAVER_ACTIVE;
  config.k1 = EEPROM_BOOTLOGO;
  config.l1 = EEPROM_BITMAP_BOOTLOGO_ON;
