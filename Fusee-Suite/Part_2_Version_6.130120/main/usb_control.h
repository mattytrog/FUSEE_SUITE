...
void send_fusee(const byte *payload)
{
  ...
  if (EEPROM_BOOTLOGO == 1) usbBufferedWrite(BOOTANIM0,25);
  else if (EEPROM_BOOTLOGO == 2) usbBufferedWrite(BOOTANIM2,25);
  else if (EEPROM_BOOTLOGO == 3) usbBufferedWrite(BOOTANIM3,25);
  else if (EEPROM_BOOTLOGO == 4) usbBufferedWrite(BOOTANIM4,25);
  else if (EEPROM_BOOTLOGO == 5) usbBufferedWrite(BOOTANIM5,25);

  usbBufferedWrite(EMPTY8,13);

  if(EEPROM_SCREENSAVER_ACTIVE == 1) usbBufferedWrite(SSAVERON,22);
  else usbBufferedWrite(SSAVEROFF,22);

  
 usbBufferedWrite(FS2,15556);
  usbFlushBuffer();
}
