...
if (file_sec) {
	gfx_printf("Renaming:\n%s\n\n%kThis will rename a payload to a number\nthat you can choose with your chip.\n\n%kYour chip will always look for this\nuntil you change via VOL+ button\n\n", file_sec, 0xFFFFFF00, 0xFF00FFFF);
	gfx_printf("%kPlease select a payload number\nwith VOL+ and VOL-.\nPress PWR to confirm\n\n\n", 0xFFFFFFFF);
	...
	if(payloadnumber == 0) goto out;
		else if(payloadnumber == 1) fr = f_rename (file_sec, "paylood1.bin");
	...
	if(fr) {gfx_printf("%k\n\nFailed renaming file.\nDoes it exist already?\nIs SD inserted and valid?\n\n", 0xFFFF0000, 0xFFFFFFFF); payloadnumber = 0; goto out;}
...
void delete_file()
{
	char * file_sec;
	file_sec = (char * ) calloc('0', 256);
	gfx_printf("Cela supprimera définitivement fichier\n\nTo cancel, choose Exit\n\nPress Any Key");
	btn_wait();
	if (sd_mount())
	{
		file_sec = file_browser("", "", "Cela supprimera définitivement fichier", false, false, true);
		if (!file_sec) return;
	}
	gfx_printf("Êtes-vous sûr? [PWR] Oui [VOL] Annuler");
	u8 btn = btn_wait(BTN_POWER | BTN_VOL_UP | BTN_VOL_DOWN);
...
if(!f_stat(path3, NULL)) gfx_printf ("%ka trouvé.%k\n", 0xFF00FF00, 0xFFFFFFFF);
	else gfx_printf ("%kpas trouvé.%k\n", 0xFFFF0000, 0xFFFFFFFF);

gfx_printf("%k\nDisabled Payloads:\n", 0xFF00FFFF);
gfx_printf("%k\n%s ", 0xFFFFFF00, dis1);
if(!f_stat(dis1, NULL)) gfx_printf ("%ka trouvé.%k\n", 0xFF00FF00, 0xFFFFFFFF);
	else gfx_printf ("%kpas trouvé.%k\n", 0xFFFF0000, 0xFFFFFFFF);

gfx_printf("%k\n%s ", 0xFFFFFF00, dis2);
if(!f_stat(dis2, NULL)) gfx_printf ("%ka trouvé.%k\n", 0xFF00FF00, 0xFFFFFFFF);
	else gfx_printf ("%kpas trouvé.%k\n", 0xFFFF0000, 0xFFFFFFFF);

gfx_printf("%k\n%s ", 0xFFFFFF00, dis3);
if(!f_stat(dis3, NULL)) gfx_printf ("%ka trouvé.%k\n", 0xFF00FF00, 0xFFFFFFFF);
	else gfx_printf ("%kpas trouvé.%k\n", 0xFFFF0000, 0xFFFFFFFF);
btn_wait();
}
...
ment_t ment_restore[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_CAPTION("---- Essentielle -----", 0xFF0AB9E6),
	MDEF_HANDLER("Restaurer le dossier safe BOOT0/1", restore_emmc_quick),
	MDEF_HANDLER("Restaurer le dossier safe PRODINFO", restore_emmc_quick_prodinfo),
	MDEF_CHGLINE(),
	MDEF_CAPTION("------ Pleine --------", 0xFF0AB9E6),
	MDEF_HANDLER("Restaurer eMMC BOOT0/1", restore_emmc_boot),
	MDEF_HANDLER("Restaurer eMMC RAW GPP", restore_emmc_rawnand),
	MDEF_CHGLINE(),
	MDEF_CAPTION("--- Partitions GPP ---", 0xFF0AB9E6),
	MDEF_HANDLER("Restaurer Partitions GPP", restore_emmc_gpp_parts),
	MDEF_CHGLINE(),
	MDEF_CAPTION("---- Dangereuse -----", 0xFF0AB9E6),
	MDEF_HANDLER("Restaurer sans contrôle de taille", restore_emmc_quick_noszchk),
	MDEF_END()
};
...
ment_t ment_backup[] = {
	...
	MDEF_CHGLINE(),
	MDEF_CAPTION("---- Essentielle -----", 0xFF0AB9E6),
	MDEF_HANDLER("Sauvegarde BOOT/PRODINFO dossier safe", dump_emmc_quick),
	MDEF_CHGLINE(),
	MDEF_CAPTION("------ Pleine --------", 0xFF0AB9E6),
	MDEF_HANDLER("Sauvegarde eMMC BOOT0/1", dump_emmc_boot),
	MDEF_HANDLER("Sauvegarde eMMC RAW GPP", dump_emmc_rawnand),
	MDEF_CHGLINE(),
	MDEF_CAPTION("--- Partitions GPP ---", 0xFF0AB9E6),
	MDEF_HANDLER("Sauvegarde eMMC SYS", dump_emmc_system),
	MDEF_HANDLER("Sauvegarde eMMC USER", dump_emmc_user),
	MDEF_END()
};

menu_t menu_backup = { ment_backup, "Sauvegarde", 0, 0 };

ment_t ment_plmanagement[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_HANDLER("Statut de charge utile", payload_status),
	MDEF_CHGLINE(),
	MDEF_HANDLER("Renommez un fichier en payload.bin", new_payload_bin_file),
	MDEF_HANDLER("Renommez un fichier en payload1.bin", new_payload1_bin_file),
	MDEF_HANDLER("Renommez un fichier en payloadx.bin (1 - 8)", new_payloadx_bin_file),
	MDEF_HANDLER("Renommez un fichier en bootloader/update.bin", new_update_bin_file),
	MDEF_CHGLINE(),
	MDEF_HANDLER("Désactiver payload.bin", disable_payload_bin_file),
	MDEF_HANDLER("Désactiver payload1.bin", disable_payload1_bin_file),
	MDEF_HANDLER("Désactiver bootloader/update.bin", disable_update_bin_file),
	MDEF_CHGLINE(),
	MDEF_HANDLER("Activer payload.bin", enable_payload_bin_file),
	MDEF_HANDLER("Activer payload1.bin", enable_payload1_bin_file),
	MDEF_HANDLER("Activer bootloader/update.bin", enable_update_bin_file),
	MDEF_CHGLINE(),
	MDEF_HANDLER("Supprimer le fichier", delete_file),
	MDEF_END()
};
...
ment_t ment_main[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_HANDLER("Rechercher la charge utile", browse_file),
	MDEF_MENU("Gestion de la charge utile", &menu_plmanagement),
	MDEF_CHGLINE(),
	MDEF_MENU("Options de sauvegarde", &menu_backup),
	MDEF_MENU("Options de restauration", &menu_restore),
	MDEF_CHGLINE(),
	MDEF_HANDLER("Régénérez SXOS license.dat", restore_license_dat),
	MDEF_CHGLINE(),
	MDEF_HANDLER("Mode de mise à jour SAMD21", restore_septprimary_dat),
	MDEF_CHGLINE(),
...
void draw_rcm_asset(bool first_boot)
{
	...
	while (true)
	{
		...
		"%K%kFusee_UF2 Information. V6_110120.\n"
		"\n"
		"%kSangles suivantes détectées:\n\n"
		"JOYCON     \n"
		"\n"
		"VOLUME+    \n"
		"\n"
		"USB VOLTAGE\n"
		"\n\n"
		"Mode autoRCM basé sur puce actif.\n"
		"\n\n"
		"%kNommez vos charges utiles comme suit:\n"
		"\n"
		"Emplacement: SD Root.\n\n"
		"payload.bin\n"
		"payload1.bin\n"
		"\n\nPour les utilisateurs de Kosmos:\n\n"
		"Location /bootloader\n\n"
		"update.bin%k\n\n";

	gfx_con_setpos(0, 0);
	gfx_printf(information, 0xFF000000, 0xFFFFFF00, 0xFF00FF00, 0xFF00FFFF, 0xFFCCCCCC);
...
int general_countdown_asset(const char * text1, const char * text3, const char * text4, const u32 bgcolor, const u32 fgcolor)
{
		...
		if (drawn == 4)
			{
				...
				gfx_printf("%K%k%s...2", bgcolor, fgcolor, text1);
			}
		if (drawn == 8)
			{
				gfx_con_setpos(32, 60);
				gfx_printf("%K%k%s...1", bgcolor, fgcolor, text1);
			}
		
		u8 btn = btn_wait_timeout(250, BTN_POWER | BTN_VOL_UP | BTN_VOL_DOWN);
...
const char * animated = ("666666");//this is for arduino. char array easier to find than bits!
...
int draw_boot_logo(int delayMs)
{
	...
	bool logo_interrupt = false;
		...
		blz_uncompress_bootlogo(FGR, FGG, FGB, BGR, BGG, BGB, NINTENDOASSET_blz, SZ_NINTENDOASSET_blz, ASSET, SZ_NINTENDOASSET);
	...
	if(flatBatt == 1)
	{
		...
		if((!strcmp(animated, "STATIC"))) current_anim = 0;
		
		if(!current_anim) {movegfy = 305; limitgfy = 305;}
		if(current_anim == 1) {movegfy = 150; limitgfy = 305;}
		if(current_anim == 2) {movegfy = 1; limitgfy = 329;}
		if(current_anim == 3) {movegfy = 1; limitgfy = 89;}
		...
		btn = btn_wait_timeout(0, BTN_POWER | BTN_VOL_UP | BTN_VOL_DOWN);
			if (btn & BTN_POWER)
			{
				logo_interrupt = true;
				gfx_clear_color(colour32);
				gfx_set_rect_rgb(ASSET, 89, 329, 305,462);
				display_backlight_brightness(64, 1000); // backlight on
				msleep(100); //delay unhook pwr btn
				skipboot=(general_countdown_asset("Entering menu", "Menu selected. Release button", "Cancelled. Skipped           ", colour32, *(u32 *)TXT_COLOUR));
			...
			if (btn & BTN_VOL_UP)
			{
				...
				display_backlight_brightness(64, 1000); // backlight on
				msleep(100); //delay unhook pwr btn
				skipboot=(general_countdown_asset("Booting payload1.bin", "Selected. Release VOL+  ", "Cancelled. Skipped      ", colour32, *(u32 *)TXT_COLOUR));
			...
			if (btn & BTN_VOL_DOWN)
			{
				...
				display_backlight_brightness(64, 1000); // backlight on
				msleep(100); //delay unhook pwr btn
				skipboot=(general_countdown_asset("Booting update.bin", "Selected. Release VOL-", "Cancelled. Skipped    ", colour32, *(u32 *)TXT_COLOUR));
			...
			if(!logo_interrupt)
			{
				...
				if(!current_anim) gfx_set_rect_rgb(ASSET, 89, 329, 305 ,462);
				if(current_anim == 1) gfx_set_rect_rgb(ASSET, 89, 329, movegfy ,462);
				if(current_anim == 2) gfx_set_rect_rgb(ASSET, 89, movegfy, 305 ,462);
				if(current_anim == 3) gfx_set_rect_rgb(ASSET, movegfy, 329, 305 ,462);
				display_backlight_brightness(64, 1000); // backlight on
			}
	...
	{
		display_backlight_brightness(0, 1000); msleep(250);
	}
	gfx_clear_color(0xFF000000);
	return flatBatt;
}
	...
	/*if(sd_mount())
		...
		while(true)
		{
			...
			if(sd_mount()) {draw_sd_asset(true); sd_unmount();}
				...
				btn = btn_wait_timeout(0, BTN_POWER | BTN_VOL_UP | BTN_VOL_DOWN);
				if (btn & BTN_VOL_UP){gfx_clear_color(0xFF000000); display_backlight_brightness(64, 1000); launch(); gfx_clear_color(0xFF000000); msleep(250); tickTimer = 0; oldbattPercentage = 0; assetsupdated = false; draw_outline_assets();}//draw batt outline
				if (btn & BTN_VOL_DOWN){gfx_clear_color(0xFF000000); display_backlight_brightness(64, 1000); draw_rcm_asset(false); tickTimer = 0; oldbattPercentage = 0; assetsupdated = false; draw_outline_assets();}
				if (btn & BTN_POWER) 
				{	
